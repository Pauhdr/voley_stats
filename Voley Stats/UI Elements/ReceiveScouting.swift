...
struct ReceiveScouting: View {
    @ObservedObject var viewModel: ReceiveScoutingModel
    var body: some View {
        VStack{
                        ...
                        HStack{
                            ZStack{
                                ...
                                if viewModel.showStats {
                                    ...
                                    Rectangle().fill(tot > 0 && err == 0 ? .green : tot > 0 && err == 0 ? .green : .red.opacity(tot > 0 ? Double(err) / Double(tot) : 0))//.padding(5)
...
class ReceiveScoutingModel: ObservableObject{
    ...
    init(team: Team, scout:Scout, rotation: [Int], player: Int, showStats: Bool, flip: Binding<Bool>){
        ...
        self.flip = flip
        Scout.teamScouts(teamName: scout.teamName, related: team, action: "receive").forEach{s in
            if player == 0 {
    ...
    func saveScout(){
        if from != -1 && to != -1 && player != 0 && !showStats{
            let sc = Scout(teamName: scout.teamName, teamRelated: team, player: self.player, rotation: self.rotation, action: "receive", difficulty: self.difficulty, from: from, to: to, date: Date())
...
//struct ReceiveScouting_Previews: PreviewProvider {
//    static var previews: some View {
//        let team = Team(name: "", organization: "", category: "", gender: "", color: .red, id: 0)
//        ReceiveScouting(viewModel: ReceiveScoutingModel(team: team, scout: Scout(teamName: "", teamRelated: team, player: 0, rotation: [0,0,0,0,0,0], action: "create", difficulty: 0, from: 0, to: 0, date: Date()), rotation: [0,0,0,0,0,0], player: 0, showStats: false))
//    }
//}
